...
public class Session : AggregateRoot
{
    ...
    {
        if (_reservations.All(reservation => reservation.ParticipantId != participantId))
        {
    ...
    private bool IsPastSession(DateTime utcNow) => (Date.ToDateTime(Time.End) - utcNow).TotalHours < 0;
    ...
    public bool HasReservationForParticipant(Guid participantId) => _reservations.Any(reservation => reservation.ParticipantId == participantId);
    ...
    public void Cancel() => _domainEvents.Add(new SessionCanceledEvent(this));

    public List<Guid> GetParticipantIds() => _reservations.ConvertAll(reservation => reservation.ParticipantId);
}
